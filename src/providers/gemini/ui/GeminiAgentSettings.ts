...
const GEMINI_AGENT_INVALID_SEGMENT_PATTERN = /[<>:"\\|?*]/;

export function validateGeminiAgentName(name: string): string | null {
  ...
  for (const segment of segments) {
    ...
    if (segment.includes('\0') || GEMINI_AGENT_INVALID_SEGMENT_PATTERN.test(segment)) {
...
export function findGeminiAgentNameConflict(
  agents: GeminiAgentDefinition[],
  name: string,
...
class GeminiAgentModal extends Modal {
  private existing: GeminiAgentDefinition | null;
  private allAgents: GeminiAgentDefinition[];
  private onSave: (agent: GeminiAgentDefinition) => Promise<void>;

  constructor(
    app: App,
    existing: GeminiAgentDefinition | null,
    allAgents: GeminiAgentDefinition[],
    onSave: (agent: GeminiAgentDefinition) => Promise<void>,
  ) {
    super(app);
  ...
  onOpen() {
    this.setTitle(this.existing ? 'Edit Gemini Subagent' : 'Add Gemini Subagent');
    ...
    new Setting(contentEl)
      .setName('Name')
      .setDesc('Gemini agent name. Use slash-separated segments for nested agents.')
      .addText((text) => {
    ...
    new Setting(contentEl)
      .setName('Description')
      .setDesc('When Gemini should use this subagent')
      .addText((text) => {
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      const nameError = validateGeminiAgentName(name);
      if (nameError) {
      ...
      const duplicate = findGeminiAgentNameConflict(
        this.allAgents,
        name,
...
export class GeminiAgentSettings {
  ...
  private storage: GeminiAgentStorage;
  ...
  async render(): Promise<void> {
    ...
    headerEl.createSpan({ text: 'Gemini Subagents', cls: 'claudian-sp-label' });
    ...
    if (visibleAgents.length === 0) {
      ...
      emptyEl.setText('No Gemini subagents in vault. Click + to create one.');
      return;
    }
  ...
  private renderItem(listEl: HTMLElement, agent: GeminiAgentDefinition): void {
  ...
  private openModal(existing: GeminiAgentDefinition | null): void {
    ...
    const modal = new GeminiAgentModal(
      this.app,
      existing,
      this.agents,
      async (agent) => {
