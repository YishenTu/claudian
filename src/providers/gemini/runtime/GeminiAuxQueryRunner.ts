...
type GeminiAuxArtifactPurpose = 'inline' | 'instructions' | 'title-gen';

interface GeminiAuxQueryRunnerOptions {
  agentProfile: GeminiAuxAgentProfile;
  artifactPurpose: GeminiAuxArtifactPurpose;
...
export class GeminiAuxQueryRunner implements AuxQueryRunner {
  ...
  constructor(
    ...
    private readonly options: GeminiAuxQueryRunnerOptions,
  ) {}
  ...
  async query(config: AuxQueryConfig, prompt: string): Promise<string> {
    ...
    if (!this.connection) {
      throw new Error('Gemini runtime is not ready.');
    }

    if (!this.sessionId) {
      const sessionId = await this.createSession(cwd);
      if (!sessionId) {
        throw new Error('Failed to create an Gemini session.');
      }
    }
    ...
    try {
      ...
      await this.connection.prompt({
        prompt: [{ type: 'text', text: `${config.systemPrompt}\n\n${prompt}` }],
        sessionId,
      });
  ...
  private async ensureReady(cwd: string): Promise<void> {
    ...
    const runtimeEnv = buildGeminiRuntimeEnv(settings, resolvedCliPath);
  ...
  private resolveSelectedRawModel(explicitModel?: string): string | undefined {
    ...
    if (explicitModel) {
      ...
      return geminiChatUIConfig.ownsModel(trimmed, projectedSettings)
        ? decodeGeminiModelId(trimmed) ?? undefined
        : trimmed;
    }
    ...
    const selectedModel = typeof projectedSettings.model === 'string'
      ...
      : '';
    return geminiChatUIConfig.ownsModel(selectedModel, projectedSettings)
      ? decodeGeminiModelId(selectedModel) ?? undefined
      : undefined;
  }
  ...
  private resolveSessionPath(sessionId: string, rawPath: string): string {
    ...
    throw new Error('Gemini aux read access is limited to the current workspace.');
  }
}
