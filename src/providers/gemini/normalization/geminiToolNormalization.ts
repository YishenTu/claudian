...
type GeminiKnownToolName = keyof typeof TOOL_NAME_MAP;

function isPlainObject(value: unknown): value is Record<string, unknown> {
  return value !== null && typeof value === 'object' && !Array.isArray(value);
}

function isKnownToolName(value: unknown): value is GeminiKnownToolName {
...
function toKnownToolName(value: string | undefined): GeminiKnownToolName | null {
  if (!value) {
    return null;
  }
  ...
  return isKnownToolName(normalized)
    ? normalized as GeminiKnownToolName
    : null;
}
...
export function resolveGeminiRawToolName(
...
export function normalizeGeminiToolName(rawName: string | undefined): string {
...
export function normalizeGeminiToolInput(
...
export function normalizeGeminiToolUseResult(
  rawName: string | undefined,
  input: Record<string, unknown>,
  rawOutput: unknown,
...
export function createGeminiToolStreamAdapter(): AcpToolStreamAdapter {
  return new AcpToolStreamAdapter({
    normalizeToolInput: normalizeGeminiToolInput,
    normalizeToolName: normalizeGeminiToolName,
    normalizeToolUseResult: normalizeGeminiToolUseResult,
    resolveRawToolName: resolveGeminiRawToolName,
  });
}
