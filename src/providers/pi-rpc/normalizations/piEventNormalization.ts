...
export function createPiEventNormalizationState(displayName: string): PiEventNormalizationState {
  return {
    displayName,
...
export function normalizePiRpcEvent(
  ...
  switch (event.type) {
    ...
    case 'message_end':
    case 'turn_end':
      return normalizeTerminalError(event, state);
    case 'compaction_end':
      return [{ type: 'context_compacted' }];
    case 'auto_retry_start':
      return [{ type: 'notice', content: `${state.displayName} is retrying the turn.`, level: 'warning' }];
    case 'auto_retry_end':
      return [{ type: 'notice', content: `${state.displayName} retry finished.`, level: 'info' }];
    case 'extension_error':
      return [{ type: 'notice', content: getString(event.error) ?? `${state.displayName} extension error.`, level: 'warning' }];
    default:
      return [];
  }
}

export function getPiTerminalErrorMessage(
  event: Record<string, unknown>,
  displayName: string,
): string | null {
...
function normalizeTerminalError(
  ...
  const message = getPiTerminalErrorMessage(event, state.displayName);
