...
export class ObsidianPiExtensionUiRenderer implements PiExtensionUiRenderer {
  ...
  private readonly headerFallback: string;
  private readonly notificationFallback: string;

  constructor(app: App, options: { displayName: string }) {
    this.app = app;
    this.headerFallback = `${options.displayName} extension`;
    this.notificationFallback = `${options.displayName} extension notification.`;
  }
  ...
  async select(
    ...
    return new PiSelectModal(this.app, request, signal, this.headerFallback).openAndWait();
  }
  ...
  async confirm(
    ...
    return new PiConfirmModal(this.app, request, signal, this.headerFallback).openAndWait();
  }
  ...
  async input(
    ...
    return new PiTextModal(this.app, request, signal, this.headerFallback, false).openAndWait();
  }
  ...
  async editor(
    ...
    return new PiTextModal(this.app, request, signal, this.headerFallback, true).openAndWait();
  }

  notify(request: PiExtensionUiNotifyRequest): void {
    new Notice(getDisplayText(request, this.notificationFallback));
  }
...
function getDisplayText(request: Record<string, unknown>, fallback: string): string {
...
abstract class PiExtensionModal<TResult extends Record<string, unknown>> extends Modal {
  ...
  constructor(
    ...
    protected readonly headerFallback: string,
  ) {
    super(app);
  }
  ...
  protected renderHeader(): void {
    this.contentEl.createEl('h2', { text: getTitle(this.request, this.headerFallback) });
...
class PiTextModal extends PiExtensionModal<{ cancelled?: boolean; value?: string }> {
  constructor(
    ...
    super(app, request, signal, headerFallback);
  }
