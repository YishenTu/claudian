...
export function parsePiSessionModel(
  ...
  for (const entry of entries) {
    ...
    if (provider && modelId) {
      selection = models.encodeModelId(provider, modelId);
    }
  }
  return selection;
}
...
export function findPiSessionFile(
  ...
  const roots = [
    sessionDir,
    cwd ? path.join(cwd, profile.agentDirName, 'agent', 'sessions') : null,
    path.join(os.homedir(), profile.agentDirName, 'agent', 'sessions'),
