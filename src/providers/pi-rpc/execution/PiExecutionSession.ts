...
implements ProviderExecutionSession, SteerableExecutionSession {
  ...
  constructor(
    ...
    this.normalizationState =
      createPiEventNormalizationState(profile.displayName);
  ...
  execute(request: ProviderExecutionRequest): ProviderExecutionRun {
    if (this.disposed) {
      throw new Error(`${this.profile.displayName} execution session is disposed`);
    }
    if (this.lifecycleError) {
      throw new Error(
        `${this.profile.displayName} execution session cleanup failed and requires disposal.`,
      );
    }
    if (this.activeRun) {
      throw new Error(
        `${this.profile.displayName} execution session already has an active run`,
      );
    }

    const active = this.createActiveRun(request);
    this.activeRun = active;
    this.normalizationState = createPiEventNormalizationState(this.profile.displayName);
  ...
  cancel(): void {
    ...
    active.terminalSignal.reject(
      new Error(`${this.profile.displayName} turn cancelled`),
    );
  ...
  private async encodeRequest(
    ...
    const settings = this.profile.settings.get(this.host.settings);
    if (!settings.enabled) {
      throw new PiConfigurationError(`${this.profile.displayName} is disabled.`);
    }
    ...
    const envText = getRuntimeEnvironmentText(this.host.settings, this.profile.providerId);
    const env = {
      ...process.env,
    ...
    const launchSpec = buildPiLaunchSpec({
      command: await this.host.getResolvedProviderCliPath(this.profile.providerId)
  ...
  private validateResumeSeed(environment: NodeJS.ProcessEnv): void {
    ...
    if (pathTarget) {
      ...
      throw new PiProviderSessionMissingError(this.profile, pathTarget);
    }
  }
  ...
  private async applyModelConfiguration(
    ...
    const modelPayload = buildPiSetModelPayload(encoded.model, this.profile.models);
    if (!modelPayload || !this.kernel) {
      throw new PiConfigurationError(
        `The selected ${this.profile.displayName} model is invalid.`,
      );
    }
  ...
  private handleRpcEvent(
    ...
    if (event.type === 'error') {
      active.terminalSignal.reject(new Error(
        getString(event.error) ?? `${this.profile.displayName} runtime error.`,
      ));
      return;
    }

    const terminalError = getPiTerminalErrorMessage(event, this.profile.displayName);
    if (terminalError) {
      this.ensureAccepted(active);
  ...
  private handleKernelClose(
    ...
    if (active && !active.terminal) {
      const runError = active.pendingTerminalError
        ?? error
        ?? new Error(`${this.profile.displayName} subprocess exited.`);
      active.pendingTerminalError = null;
      ...
      this.setInvalidated({
        message: error?.message ?? `${this.profile.displayName} subprocess exited.`,
  ...
  private async refreshState(signal: AbortSignal): Promise<void> {
    if (!this.kernel) {
      throw new Error(
        `${this.profile.displayName} execution kernel is unavailable.`,
      );
    }
  ...
  private async fetchUsage(model: string, signal: AbortSignal) {
    ...
    const settings = this.profile.settings.get(this.host.settings);
    const contextWindow = this.profile.models.findModel(settings, model)?.contextWindow;
  ...
  private async materializePendingForkForRun(active: ActiveRun): Promise<void> {
    ...
    const sourceFile = state.forkSourceSessionFile
      ?? findPiSessionFile(
        ...
        this.config.vaultWorkingDirectory,
        getString(env[this.profile.sessionDirEnvKey]),
      );
    if (!sourceFile) {
      throw new Error(
        `${this.profile.displayName} fork source session not found: ${forkSource.sessionId}`,
      );
    }
    ...
    if (isSamePath(fork.sessionFile, sourceFile)) {
      throw new Error(
        `${this.profile.displayName} fork materialization returned the source session as its target.`,
      );
    }
    if (!this.isActive(active) || active.abortController.signal.aborted) {
  ...
  private finishError(
    ...
    active.terminalSignal.reject(
      error instanceof Error ? error : new Error(`${this.profile.displayName} execution failed.`),
    );
...
class PiProviderSessionMissingError extends Error {
  constructor(profile: PiFamilyProfile, readonly providerSessionId: string) {
    super(`${profile.displayName} session is unavailable: ${providerSessionId}`);
    this.name = 'PiProviderSessionMissingError';
  }
}

class PiProviderSessionMismatchError extends Error {
  constructor(profile: PiFamilyProfile, expected: string, reported: string | null) {
    super(
      `${profile.displayName} resumed an unexpected native session.`
        + ` Expected ${expected}, received ${reported ?? 'no session identity'}.`
        + ' The original conversation session was preserved.',
    );
...
function resolveSelectedModel(
  ...
  if (
    !model
    || !profile.models.decodeModelId(model)
    || !settings.visibleModels.includes(model)
  ) {
    throw new PiConfigurationError(
      `No ${profile.displayName} model is selected.`
        + ' Enable a discovered model in Claudian settings.',
    );
  }
  return model;
}
...
function resolveThinkingLevel(
  ...
  const discovered = profile.models.findModel(settings, model);
  return discovered
    ? profile.models.clampThinkingLevel(requested, discovered.thinkingLevels)
    : requested ?? null;
}
...
function classifyError(
  ...
  const baseMessage = error instanceof Error
    ? error.message
    : `${profile.displayName} execution failed.`;
