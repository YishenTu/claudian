...
export interface PiFamilyModelContract {
  readonly defaultThinkingLevel: PiThinkingLevel;
  readonly modelIdPrefix: string;
  clampThinkingLevel: typeof clampPiThinkingLevel;
  decodeModelId: (model: string) => DecodedPiModelId | null;
  encodeModelId: (provider: string, modelId: string) => string;
  findModel: typeof findPiModel;
  isModelSelectionId: (model: string) => boolean;
  normalizeDiscoveredModels: (value: unknown) => PiDiscoveredModel[];
  normalizeThinkingLevel: typeof normalizePiThinkingLevel;
  getSupportedThinkingLevels: typeof getPiSupportedThinkingLevels;
}

export function definePiFamilyModels(modelIdPrefix: string): PiFamilyModelContract {
  const encodeModelId = (provider: string, modelId: string): string => {
    ...
    return `${modelIdPrefix}${normalizedProvider}/${normalizedModelId}`;
  };

  const decodeModelId = (model: string): DecodedPiModelId | null => {
    if (!model.startsWith(modelIdPrefix)) {
      return null;
    }

    const raw = model.slice(modelIdPrefix.length).trim();
  ...
  return {
    ...
    modelIdPrefix,
    clampThinkingLevel: clampPiThinkingLevel,
    decodeModelId,
    encodeModelId,
    findModel: findPiModel,
    isModelSelectionId: model => decodeModelId(model) !== null,
    normalizeDiscoveredModels: value => normalizePiFamilyDiscoveredModels(value, encodeModelId),
    normalizeThinkingLevel: normalizePiThinkingLevel,
    getSupportedThinkingLevels: getPiSupportedThinkingLevels,
  };
}

function normalizePiFamilyDiscoveredModels(
  value: unknown,
  encodeModelId: (provider: string, modelId: string) => string,
): PiDiscoveredModel[] {
  ...
  for (const entry of value) {
    ...
    normalized.push({
      ...
      encodedId: encodeModelId(provider, id),
      id,
      input,
      label,
