...
export class PiSubprocess {
  ...
  constructor(launchSpec: PiSubprocessLaunchSpec, private readonly profile: PiFamilyProfile) {
    const enhancedPath = getEnhancedPath(
      launchSpec.env.PATH,
      path.isAbsolute(launchSpec.command) ? launchSpec.command : undefined,
    );
    const processSpec = resolvePiProcessSpec(launchSpec, enhancedPath, this.profile);
...
> {
  ...
  if (command.toLowerCase().endsWith('.cmd')) {
    nodeEntrypoint = resolveInstalledPiBin(command, profile);
    if (!nodeEntrypoint) {
      throw new Error(
        `The ${profile.displayName} Windows launcher could not be resolved to its Node.js entry point: ${command}`,
      );
    }
  ...
  if (!nodeExecutable) {
    throw new Error(
      `${profile.displayName} requires Node.js, but node.exe was not found on PATH. Install Node.js or configure a native ${profile.displayName} executable.`,
    );
  }
  return {
...
function resolveInstalledPiBin(command: string, profile: PiFamilyProfile): string | null {
  if (path.basename(command).toLowerCase() !== profile.windowsShimName) return null;

  const shimTarget = readWindowsCommandShimTarget(command);
  return shimTarget ? resolveOwningPiPackageBin(shimTarget, profile) : null;
}
...
function resolveOwningPiPackageBin(target: string, profile: PiFamilyProfile): string | null {
  const resolvedTarget = path.resolve(target);
  let current = path.dirname(resolvedTarget);
  while (true) {
    const binPath = readPiPackageBin(current, profile);
...
function readPiPackageBin(packageRoot: string, profile: PiFamilyProfile): string | null {
  try {
    ...
    if (packageJson.name !== profile.windowsPackageName) return null;
    const binEntry = typeof packageJson.bin === 'object' && packageJson.bin !== null
      ? packageJson.bin[profile.windowsBinKey]
      : undefined;
    const relativeBin = typeof binEntry === 'string' ? binEntry : null;
