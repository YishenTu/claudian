...
export class InstructionModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.addClass('geminian-instruction-modal');
    this.setTitle('Add Custom Instruction');

    // User input section (always visible)
    const inputSection = contentEl.createDiv({ cls: 'geminian-instruction-section' });
    const inputLabel = inputSection.createDiv({ cls: 'geminian-instruction-label' });
    inputLabel.setText('Your input:');
    const inputText = inputSection.createDiv({ cls: 'geminian-instruction-original' });
    inputText.setText(this.rawInstruction);

    // Main content section (changes based on state)
    this.contentSectionEl = contentEl.createDiv({ cls: 'geminian-instruction-content-section' });

    // Loading state
    this.loadingEl = this.contentSectionEl.createDiv({ cls: 'geminian-instruction-loading' });
    this.loadingEl.createDiv({ cls: 'geminian-instruction-spinner' });
    this.loadingEl.createSpan({ text: 'Processing your instruction...' });

    // Clarification state (hidden initially)
    this.clarificationEl = this.contentSectionEl.createDiv({ cls: 'geminian-instruction-clarification-section' });
    this.clarificationEl.style.display = 'none';
    this.clarificationTextEl = this.clarificationEl.createDiv({ cls: 'geminian-instruction-clarification' });

    const responseSection = this.clarificationEl.createDiv({ cls: 'geminian-instruction-section' });
    const responseLabel = responseSection.createDiv({ cls: 'geminian-instruction-label' });
    responseLabel.setText('Your response:');

    this.responseTextarea = new TextAreaComponent(responseSection);
    this.responseTextarea.inputEl.addClass('geminian-instruction-response-textarea');
    ...
    this.confirmationEl = this.contentSectionEl.createDiv({ cls: 'geminian-instruction-confirmation-section' });
    this.confirmationEl.style.display = 'none';

    // Refined instruction display/edit
    const refinedSection = this.confirmationEl.createDiv({ cls: 'geminian-instruction-section' });
    const refinedLabel = refinedSection.createDiv({ cls: 'geminian-instruction-label' });
    refinedLabel.setText('Refined snippet:');

    this.refinedDisplayEl = refinedSection.createDiv({ cls: 'geminian-instruction-refined' });
    this.editContainerEl = refinedSection.createDiv({ cls: 'geminian-instruction-edit-container' });
    this.editContainerEl.style.display = 'none';

    this.editTextarea = new TextAreaComponent(this.editContainerEl);
    this.editTextarea.inputEl.addClass('geminian-instruction-edit-textarea');
    this.editTextarea.inputEl.rows = 4;

    // Buttons (changes based on state)
    this.buttonsEl = contentEl.createDiv({ cls: 'geminian-instruction-buttons' });
    this.updateButtons();
  ...
  showClarificationLoading() {
    ...
    if (this.loadingEl) {
      this.loadingEl.querySelector('.geminian-instruction-spinner');
  ...
  private updateButtons() {
    ...
    const cancelBtn = this.buttonsEl.createEl('button', {
      text: 'Cancel',
      cls: 'geminian-instruction-btn geminian-instruction-reject-btn',
    ...
    if (this.state === 'clarification') {
      const submitBtn = this.buttonsEl.createEl('button', {
        text: 'Submit',
        cls: 'geminian-instruction-btn geminian-instruction-accept-btn',
      ...
      this.editBtnEl = this.buttonsEl.createEl('button', {
        text: 'Edit',
        cls: 'geminian-instruction-btn geminian-instruction-edit-btn',
