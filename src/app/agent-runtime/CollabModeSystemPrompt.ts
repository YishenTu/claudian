...
export function buildCollabModeSystemPrompt(
  ...
  return [
    ...
    'Use collab.projects.get to inspect Update availability. Unknown or offline state does not prove that an update is available. A sync-required state means team changes are already included and only Project status needs syncing. Follow nextAction: publish-pending requires completing the existing publication before Update. Offline state may retain a local Update conflict or recovery task, but continuation requires reconnection.',
