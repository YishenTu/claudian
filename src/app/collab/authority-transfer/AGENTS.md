...
- Claimant recovery is reconstructed by the app composition from the surviving local membership plus the claimant record. Cloud-to-LAN records retain only the public pinned LAN target endpoint and CA trust needed for restart; no claimant operation may depend on a previously bound live runtime. At expiry, pre-redemption records are scrubbed without remote replay, while a completed target redemption recovers forward through local convergence without retrying the expired source acknowledgement. A membership-converged record is terminal cleanup and never resolves a transport.
...
- Source keys, proofs, and manifests are written through operation-owned temporary files and atomic promotion. A restart removes or replaces only the exact operation partial and never treats a partially written final artifact as durable success.
