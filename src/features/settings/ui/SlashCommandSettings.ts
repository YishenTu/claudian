...
export class SlashCommandModal extends Modal {
  ...
  onOpen() {
    const existingIsSkill = this.existingCmd
      ? isSkill(this.existingCmd)
      : false;
    let selectedType: "command" | "skill" = existingIsSkill
      ? "skill"
      : "command";

    const typeLabel = () =>
      selectedType === "skill" ? "Skill" : "Slash Command";

    this.setTitle(
      this.existingCmd ? `Edit ${typeLabel()}` : `Add ${typeLabel()}`,
    );
    ...
    let disableModelToggle: boolean =
      this.existingCmd?.disableModelInvocation ?? false;
    let disableUserInvocation: boolean =
      this.existingCmd?.userInvocable === false;
    ...
    new Setting(contentEl)
      ...
      .addText((text) => {
        ...
        text
          .setValue(this.existingCmd?.name || "")
    ...
    if (
      this.existingCmd?.argumentHint ||
      this.existingCmd?.model ||
      this.existingCmd?.allowedTools?.length ||
      this.existingCmd?.disableModelInvocation ||
      this.existingCmd?.userInvocable === false ||
      this.existingCmd?.context ||
      this.existingCmd?.agent
    ) {
      details.open = true;
    }
    ...
    new Setting(details)
      ...
      .addText((text) => {
        ...
        text
          .setValue(this.existingCmd?.model || "")
    ...
    const agentSetting = new Setting(details)
      ...
      .addText((text) => {
        ...
        text
          .setValue(this.existingCmd?.agent || "")
          .setPlaceholder("code-reviewer");
      });
    agentSetting.settingEl.style.display =
      contextValue === "fork" ? "" : "none";
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      const cmd: SlashCommand = {
        ...
        argumentHint:
          normalizeArgumentHint(hintInput.value.trim()) ||
          parsed.argumentHint ||
          undefined,
        ...
        agent:
          contextValue === "fork"
            ? agentInput.value.trim() || undefined
            : undefined,
      };
...
export class SlashCommandSettings {
  ...
  private render(): void {
    ...
    headerEl.createSpan({
      text: t("settings.slashCommands.name"),
      cls: "claudian-sp-label",
    });
  ...
  private async reloadCommands(): Promise<void> {
    this.plugin.settings.slashCommands =
      await this.plugin.storage.loadAllSlashCommands();
  }
