...
export class NavigationSidebar {
  ...
  constructor(
    ...
    this.container = this.parentEl.createDiv({ cls: 'geminian-nav-sidebar' });

    // Create buttons
    this.topBtn = this.createButton('geminian-nav-btn-top', 'chevrons-up', 'Scroll to top');
    this.prevBtn = this.createButton('geminian-nav-btn-prev', 'chevron-up', 'Previous message');
    this.nextBtn = this.createButton('geminian-nav-btn-next', 'chevron-down', 'Next message');
    this.bottomBtn = this.createButton('geminian-nav-btn-bottom', 'chevrons-down', 'Scroll to bottom');
  ...
  private createButton(cls: string, icon: string, label: string): HTMLElement {
    const btn = this.container.createDiv({ cls: `geminian-nav-btn ${cls}` });
    setIcon(btn, icon);
  ...
  private scrollToMessage(direction: 'prev' | 'next'): void {
    const messages = Array.from(this.messagesEl.querySelectorAll('.geminian-message-user')) as HTMLElement[];
