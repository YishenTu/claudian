...
export class InlineExitPlanMode {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'geminian-plan-approval-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'geminian-plan-inline-title' });
    titleEl.setText('Plan complete');

    this.planContent = this.readPlanContent();
    if (this.planContent) {
      const contentEl = this.rootEl.createDiv({ cls: 'geminian-plan-content-preview' });
      if (this.renderContent) {
        void this.renderContent(contentEl, this.planContent);
      } else {
        contentEl.createDiv({ cls: 'geminian-plan-content-text', text: this.planContent });
      }
    } else if (this.planReadError) {
      this.rootEl.createDiv({
        cls: 'geminian-plan-content-preview geminian-plan-read-error',
    ...
    if (allowedPrompts && Array.isArray(allowedPrompts) && allowedPrompts.length > 0) {
      const permEl = this.rootEl.createDiv({ cls: 'geminian-plan-permissions' });
      permEl.createDiv({ text: 'Requested permissions:', cls: 'geminian-plan-permissions-label' });
      const listEl = permEl.createEl('ul', { cls: 'geminian-plan-permissions-list' });
      for (const perm of allowedPrompts) {
        listEl.createEl('li', { text: perm.prompt });
      }
    }

    const actionsEl = this.rootEl.createDiv({ cls: 'geminian-ask-list' });

    const newSessionRow = actionsEl.createDiv({ cls: 'geminian-ask-item' });
    newSessionRow.addClass('is-focused');
    newSessionRow.createSpan({ text: '\u203A', cls: 'geminian-ask-cursor' });
    newSessionRow.createSpan({ text: '1. ', cls: 'geminian-ask-item-num' });
    newSessionRow.createSpan({ text: 'Approve (new session)', cls: 'geminian-ask-item-label' });
    ...
    const approveRow = actionsEl.createDiv({ cls: 'geminian-ask-item' });
    approveRow.createSpan({ text: '\u00A0', cls: 'geminian-ask-cursor' });
    approveRow.createSpan({ text: '2. ', cls: 'geminian-ask-item-num' });
    approveRow.createSpan({ text: 'Approve (current session)', cls: 'geminian-ask-item-label' });
    ...
    const feedbackRow = actionsEl.createDiv({ cls: 'geminian-ask-item geminian-ask-custom-item' });
    feedbackRow.createSpan({ text: '\u00A0', cls: 'geminian-ask-cursor' });
    feedbackRow.createSpan({ text: '3. ', cls: 'geminian-ask-item-num' });
    ...
    this.rootEl.createDiv({ text: HINTS_TEXT, cls: 'geminian-ask-hints' });
