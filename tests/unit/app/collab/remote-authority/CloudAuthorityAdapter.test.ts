...
describe('CloudAuthorityAdapter', () => {
  ...
  it('preserves the HTTPS prefix with native certificate verification for JSON, artifacts, and WebSockets', async () => {
    ...
    try {
      ...
      const session = await new CloudAuthorityAdapter(cloudVaultRoot, {
        ...
        authority: {
          ...bound.authority,
          gitRemoteUrl: `https://127.0.0.1:${address.port}/operator/cloud/v7/projects/project-cloud/repository.git`,
          serverUrl,
        },
      });
      try {
        ...
        expect(observed).toEqual([
          ...
          { actor: undefined, path: '/operator/cloud/v7/projects/project-cloud/operations/getProjectSnapshot' },
          { actor: undefined, path: '/operator/cloud/v7/projects/project-cloud/authority-transfers/transfer-one/checkpoint/checkpoint.json' },
          { actor: undefined, path: '/operator/cloud/v7/projects/project-cloud/events?afterSequence=3' },
        ]);
      } finally {
        session.dispose();
      }
    ...
    try {
      const pending = kind === 'bound'
        ? adapter.create({
          ...
          authority: {
            ...bound.authority,
            gitRemoteUrl: `${serverUrl}/v7/projects/project-cloud/repository.git`,
            serverUrl,
          },
  ...
  it('owns and closes its prefixed WebSocket without a development assertion', async () => {
    ...
    try {
      ...
      expect(observed).toEqual({
        actor: undefined,
        path: '/operator/cloud/v7/projects/project-cloud/events?afterSequence=3',
      });
      session.dispose();
  ...
  it.each(['upload', 'download'] as const)('aborts a prefixed artifact %s when the unbound connection is disposed', async direction => {
    ...
    try {
      ...
      expect(observed).toEqual({
        actor: undefined,
        path: '/operator/cloud/v7/projects/project-cloud/authority-transfers/transfer-one/checkpoint/checkpoint.json',
      });
    } finally {
      connection.dispose();
      server.closeAllConnections();
  ...
  it.each([
    ...
    { bindingVersion: 2 },
    { wireVersion: 6 },
    { gitRemoteUrl: 'https://other.example.test/v7/projects/project-cloud/repository.git' },
  ...
  it('uses the configured HTTP prefix without asserting a development identity', async () => {
    ...
    if (!address || typeof address === 'string') throw new Error('Missing test listener');
    const serverUrl = `HTTP://127.0.0.1:${address.port}/operator/cloud`;
    const gitRemoteUrl = `http://127.0.0.1:${address.port}/operator/cloud/v7/projects/project-cloud/repository.git`;
    const bound = membership();
  ...
  it('opens one exact bound authority-transfer connection without a snapshot preflight', async () => {
    ...
    expect(requests.map(input => input.url)).toEqual([
      'https://cloud.example.test/collab/capabilities',
      `https://cloud.example.test/v7/projects/${PROJECT_ID}`
  ...
  it('rejects a bound session before exposing mutation ports when its authenticated snapshot has another authority generation', async () => {
    ...
    expect(requests.map(input => input.url)).toEqual([
      'https://cloud.example.test/collab/capabilities',
      `https://cloud.example.test/v7/projects/${PROJECT_ID}/operations/getProjectSnapshot`,
    ]);
  });
    ...
    try {
      ...
      try {
        ...
        expect(received).toEqual([{
          ...
          path: '/operator/cloud/v7/projects/project-cloud/operations/createCloudProject',
        }]);
  ...
  it('uses the desktop transport for default capability and snapshot reads', async () => {
    ...
    const localMembership = {
      ...
      authority: {
        ...
        gitRemoteUrl: `http://127.0.0.1:${address.port}/v7/projects/project-cloud/repository.git`,
        serverUrl: `http://127.0.0.1:${address.port}`,
      },
    ...
    try {
      ...
      expect(requests).toEqual([
        ...
        {
          actor: undefined,
          url: `/v7/projects/${PROJECT_ID}/operations/getProjectSnapshot`,
        },
      ]);
      session.dispose();
    } finally {
  ...
  it('negotiates package capabilities and maps the strict Cloud snapshot', async () => {
    ...
    expect(session.git).toEqual({
      ...
      remoteUrl: `https://cloud.example.test/v7/projects/${PROJECT_ID}/repository.git`,
    });
    expect(requests).toEqual([
      ...
      expect.objectContaining({
        ...
        method: 'POST',
        url: `https://cloud.example.test/v7/projects/${PROJECT_ID}/operations/getProjectSnapshot`,
      }),
    ]);
  });
  ...
  it('ensures the current member Request through the package-owned Cloud operation', async () => {
    ...
    expect(requests[2]).toEqual({
      ...
      url: `https://cloud.example.test/v7/projects/${PROJECT_ID}/operations/ensureMyRequest`,
    });
  });
  ...
  it('routes Accept through the canonical Cloud operation with its exact authority tuple', async () => {
    ...
    expect(request.mock.calls[2]?.[0]).toEqual({
      ...
      method: 'POST',
      signal: expect.any(AbortSignal),
      url: `https://cloud.example.test/v7/projects/${PROJECT_ID}/operations/acceptRequest`,
    });
  });
    ...
    const client = new CloudProjectEventClient({
      ...
      createSocket: input => {
        ...
        expect(input).toEqual({
          headers: {},
          url: `wss://cloud.example.test/v7/projects/${PROJECT_ID}/events?afterSequence=${
