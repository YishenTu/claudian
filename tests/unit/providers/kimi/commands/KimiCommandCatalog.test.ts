...
describe('KimiCommandCatalog', () => {
  ...
  it('starts empty and prefers an explicit runtime snapshot over the cached one', async () => {
    const catalog = new KimiCommandCatalog();

    await expect(catalog.listDropdownEntries({ includeBuiltIns: true })).resolves.toEqual([]);

    catalog.setRuntimeCommands([
      { id: 'cached', name: 'cached', description: 'Cached', content: '' },
    ]);
    await expect(catalog.listDropdownEntries({
      includeBuiltIns: true,
      runtimeCommands: [{ id: 'live', name: 'live', description: 'Live', content: '' }],
    })).resolves.toEqual([expect.objectContaining({ id: 'live' })]);
