...
import { PiCommandMetadataProbe } from '@/providers/pi-rpc/execution/PiCommandMetadataProbe';
...
describe('PiCommandMetadataProbe', () => {
  it('registers a load before a synchronously started transition quiesces', async () => {
    ...
    const probe = new PiCommandMetadataProbe(piFamilyProfile, host, createKernel);
  ...
  it('falls back to a pushed command catalog when get_commands is unsupported', async () => {
    ...
    const createKernel = jest.fn((_profile, _spec, callbacks) => {
