...
} from '@/providers/pi-rpc/execution';
import { PiConversationHistoryService } from '@/providers/pi-rpc/history/PiConversationHistoryService';
import type { PiLaunchSpec } from '@/providers/pi-rpc/runtime/PiLaunchSpec';
...
function createHarness(
  ...
  const backend = new PiExecutionBackend(
    ...
    { commandCatalog },
    {
      createKernel: (_profile, spec, callbacks) => {
...
describe('PiExecutionBackend', () => {
  it('uses an isolated no-session kernel for command metadata probes', async () => {
    ...
    const probe = new PiCommandMetadataProbe(
      piFamilyProfile,
      createHost(),
      (_profile, spec, callbacks, suppliedRenderer) => {
  ...
  it('fences command metadata started during a transition and reopens on the new CLI', async () => {
    ...
    host.getResolvedProviderCliPath.mockImplementation(async () => cliPath);
    const kernels: FakeKernel[] = [];
    const probe = new PiCommandMetadataProbe(piFamilyProfile, host, (_profile, spec, callbacks) => {
      const kernel = new FakeKernel(
        spec,
        callbacks,
  ...
  it('resets a missing persisted path through the coordinator before retrying', async () => {
    ...
    const historyService = new PiConversationHistoryService(piFamilyProfile);
  ...
  it('does not duplicate recovered history after the fresh native session reloads', async () => {
    ...
    await new PiConversationHistoryService(piFamilyProfile).hydrateConversationHistory(
      conversation,
      sessionDir,
      {
        environment: {
