...
jest.mock('@/providers/pi-rpc/runtime/PiRpcTransport', () => ({
...
jest.mock('@/providers/pi-rpc/runtime/PiSubprocess', () => ({
...
import { PiModelDiscoveryService } from '@/providers/pi-rpc/runtime/PiModelDiscoveryService';
...
describe('PiModelDiscoveryService', () => {
  ...
  it('does not launch Pi when the provider is disabled', async () => {
    ...
    const result = await new PiModelDiscoveryService(piFamilyProfile, plugin).discoverModels();
  ...
  it('discovers and normalizes Pi models through a short-lived no-session runtime', async () => {
    ...
    const result = await new PiModelDiscoveryService(piFamilyProfile, createPlugin()).discoverModels();
  ...
  it('cancels extension UI requests during discovery', async () => {
    ...
    await new PiModelDiscoveryService(piFamilyProfile, createPlugin()).discoverModels();
