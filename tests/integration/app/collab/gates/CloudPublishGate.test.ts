  ...
  const server = createServer((request, response) => {
    void (async () => {
      ...
      if (
        ...
        const prefix = `/v8/projects/${PROJECT_ID}/repository.git`;
...
function membership(
  ...
  return {
    authority: {
      authorityGeneration: 1,
      bindingVersion: 8,
      gitRemoteUrl: `${origin}/v8/projects/${PROJECT_ID}/repository.git`,
      kind: 'cloud',
      serverUrl: origin,
      wireVersion: 12,
    },
    ...
    try {
      ...
      for (const actor of ACTORS) {
        ...
        await git(repositoryPath, [
          ...
          'set-url',
          'origin',
          `${server.origin}/v8/projects/${PROJECT_ID}/repository.git`,
        ]);
