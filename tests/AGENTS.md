...
- Express expiries and injected clocks with `@test/helpers/testClock`, and pin `now` wherever code under test compares them. CI runs the selected Jest tests about three years ahead under libfaketime, so anything that still reads the real clock against a fixed date fails there.
